...
async def UploadFile(payload,folder : Union[str, None], token : str) -> bool:
    ...
    # 计算size
    size = 0
    with open(upload_url + f"/{token}/" + f"{temp_filename}-{folder}","wb") as file:
        while True:
    ...
    # 重命名
    os.rename(upload_url + f"/{token}/" + f"{temp_filename}-{folder}", upload_url + f"/{token}/" + f"{filename}-{folder}")
    #若size为0，说明文件未正常传输或为空文件
    if size == 0:
        os.remove(upload_url + f"/{token}/" + f"{filename}-{folder}")
        return False,None
